...
public partial class PlayerCrosshair : Behaviour
{
    ...
    protected override void _BehaviourProcess(double delta)
    {
        ...
        if (raycast.IsColliding()) { crosshair.GlobalPosition = crosshair.GlobalPosition.Lerp(raycast.GetCollisionPoint(), (float)delta * 20f); }
        else
        { crosshair.GlobalPosition = crosshair.GlobalPosition.Lerp(raycast.ToGlobal(raycast.TargetPosition), (float)delta * 20f); }
